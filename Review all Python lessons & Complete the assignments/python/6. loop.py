...
for temp in temperatures:
    if temp > threshold:
        print(f"Warning: Temperature {temp} exceeds the threshold of {threshold} degrees.")
...
products = {
    ...
    "tablet": {"stock": 7, "min_required": 8},
    "headphones": {"stock": 20, "min_required": 10}
}
...
for product, details in products.items():
    ...
    if stock < min_required:
        print(f"Restock alert: {product} stock is low ({stock}). Please restock soon.")
    else:
        print(f"{product} stock is sufficient ({stock}).")
