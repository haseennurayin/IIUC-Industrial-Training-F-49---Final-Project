...
with open(products_csv, 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Product", "Price", "Stock"])
    writer.writerow(["Laptop", 1000, 20])
    writer.writerow(["Smartphone", 700, 50])
    writer.writerow(["Tablet", 300, 100])
...
with open(products_csv, 'r') as file:
    ...
    headers = next(reader)  
    for row in reader:
        product_info = {headers[i]: row[i] for i in range(len(headers))}
...
write_log("Starting the application.")
